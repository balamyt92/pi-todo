...
/**
 * Снимок состояния, который каждый вызов `todo` возвращает в `details` —
 * и успешный, и отклонённый. `replay.ts` читает последний валидный снимок
 * из ветки, чтобы восстановить состояние.
 *
 * Порядок и имена полей зафиксированы ради совместности между версиями и с
 * `@juicesharp/rpiv-todo`.
 *
 * Стоимость дизайна last-write-wins: полный снимок `tasks` и полные `params`
 * пишутся в КАЖДЫЙ tool result. Файл сессии растёт как
 * `O(число_вызовов × число_задач)`. Это осознанная сделка: альтернатива
 * (дельты + их свёртка при replay) ломает совместимость с уже накопленной
 * историей и требует версионирования формата дельт. При типичных сотнях
 * вызовов это десятки килобайт на диске — несущественно рядом с контекстом
 * модели, который снимок в `details` никогда не занимает.
 */
 ...
 *
 * Поля только для чтения наружу. Инкапсуляция держится типом, а не
 * договорённостью: `store.getState()` отдаёт тот же экземпляр, что лежит в
 * `Map`, и без `readonly` читатель мог бы мутировать чужую ячейку. Все
 * изменения проходят через редьюсер, который возвращает НОВЫЕ массивы.
 */
